            ...
            else
            {
                if (callBack == null)
                {
                    callBack = delegate (object o) {
                        VGen0 method = null;
                        if (new QuazalQuery("HostNewSpaceSiegeGame", new object[] { this.tbGameName.Text, this.tbPassword.Text }).ExecuteNonQuery())
                        {
                            if (method == null)
                            {
                                method = delegate {
                        ...
                        {
                            DlgMessage.ShowDialog("<LOC>Error creating game session.", "<LOC>Error");
                        }
                    };
                }
